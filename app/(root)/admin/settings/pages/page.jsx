...
const ShowPages = () => {
    ...
    const action = useCallback((row, deleteType, handleDelete) => {
        let actionMenu = []
        const editHref = `/admin/settings/pages/create/builder?edit=${row.original._id}`
        if (editHref) {
            ...
            <Card className="rounded-xl shadow-sm border border-border/60 overflow-hidden gap-0 py-0">
                <CardHeader className="px-5 py-4 border-b bg-muted/30">
                    <div className="flex justify-between items-center">
                        <h4 className='text-sm font-semibold text-foreground'>All Pages</h4>
                        <Button asChild>
                            <Link href="/admin/settings/pages/create/builder">
                                <FiPlus />
                                Create Page
                            </Link>
                        </Button>
