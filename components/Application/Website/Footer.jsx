...
const Footer = () => {
    const { data: footerData } = useFetch('/api/settings/footer', 'GET')
    const [footer, setFooter] = useState(null)

    useEffect(() => {
        if (footerData && footerData.success) {
    ...
    return (
        <footer className='bg-white border-t border-slate-100 mt-20 font-sans'>
            {footerContent ? (
                <div className="py-16 lg:px-32 px-6" dangerouslySetInnerHTML={{ __html: footerContent }} />
            ) : (
                <>
                    ...
                    <div className='grid lg:grid-cols-12 md:grid-cols-2 grid-cols-2 gap-y-12 gap-x-4 py-16 lg:px-32 px-6'>
                        ...
                        <div className='col-span-2 lg:col-span-4 md:col-span-2'>
                            ...
                            <p className='text-slate-500 text-sm leading-relaxed max-w-sm'>
                                {footer?.title || 'আল-হিলাল পাঞ্জাবি মানেই আভিজাত্য। আমরা পাইকারি ও খুচরা বিক্রয়কারী হিসেবে দেশের সেরা প্রিমিয়াম পাঞ্জাবি ও পায়জামা সরবরাহ করছি।'}
                            </p>
                            ...
                            <div className='flex gap-3 mt-8'>
                                {socialLinks.map((social, i) => {
                                    ...
                                    return (
                                        <Link key={i} href={social.url} className={`p-2.5 rounded-xl bg-slate-50 text-slate-600 hover:text-white ${socialColors[social.platform] || 'hover:bg-blue-600'} transition-all duration-300 ${trelloShadow}`}>
                                            <Icon size={18} />
                                        </Link>
                                    )
                                })}
