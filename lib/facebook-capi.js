...
/**
 * Process event queue and send batch to Facebook
 */
async function processBatch() {
  ...
  try {
    ...
    const events = batch.map(item => ({
      ...
      event_source_url: item.eventSourceUrl || 'https://alhilalpanjabi.com',
...
/**
 * Send event immediately (without batching) with retry logic
  ...
  while (retryCount < MAX_RETRIES) {
    try {
      ...
      const eventPayload = {
        ...
        event_source_url: options.eventSourceUrl || 'https://alhilalpanjabi.com',
