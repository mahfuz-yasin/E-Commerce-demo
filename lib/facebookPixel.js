...
// ============================================
// Advanced Event Match Quality Helpers
// ============================================

/**
 * Read _fbc cookie (Facebook Click ID) — NOT hashed, sent as-is to CAPI
 * Format: fb.{version}.{creationTime}.{fbclid}
 */
export const getFbc = () => {
...
/**
 * Read _fbp cookie (Facebook Browser ID) — NOT hashed, sent as-is to CAPI
 * Format: fb.{version}.{creationTime}.{randomId}
 */
export const getFbp = () => {
  if (!isBrowser) return null
...
/**
 * Generate fbc from fbclid query param if _fbc cookie not present
 * @param {string} fbclid - fbclid from URL
 */
export const generateFbc = (fbclid) => {
  if (!fbclid) return null
  return `fb.1.${Date.now()}.${fbclid}`
}

/**
 * Get all Advanced Event Match Quality data from browser
 * Use this when creating an order to pass to the server
 */
export const getAdvancedMatchData = () => {
  if (!isBrowser) return {}
  const urlParams = new URLSearchParams(window.location.search)
  const fbclid = urlParams.get('fbclid')
  return {
    fbc: getFbc() || (fbclid ? generateFbc(fbclid) : null),
    fbp: getFbp(),
